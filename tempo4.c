...
// Descrição resumida: Quarta tarefa de simulação da disciplina Sistemas Distribuídos.
...
int main(int argc, char* argv[]) {
	...
	if(argc!=2) {
		puts("Uso correto: tempo4 <número de processos>");
		exit(1);
	}
	...
	for (i=0;i<N;i++) {
		processo[i].State[i]=correto; // inicializa o estado do proprio processo como zero (cada processo acredita que está sempre funcionando)
	}
	...
	while(time()<150.0) {
		...
		switch(event) {
			case test:
				...
				for (i=num_proximo_processo+1;i<(N+num_processo_atual);i++)
				{
					if (processo[num_proximo_processo].State[i%N]!=desconhecido)
					{
